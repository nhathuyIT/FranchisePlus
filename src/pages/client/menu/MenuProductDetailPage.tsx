...
const MenuProductDetailPage = () => {
  ...
  const categoryTabs = useMemo(() => {
    ...
    console.log(menuDataAll);
    
    
    // Calculate total count across all categories
    const totalCount = menuDataAll.reduce((acc, cat) => 
      acc + cat.products.filter(p => p.sizes.some(s => s.isAvailable)).length, 0
    );
    ...
    categories?.forEach((category) => {
      const menuCategory = menuDataAll.find(m => String(m.categoryId) === String(category.categoryId));
      const count = menuCategory ? menuCategory.products.filter(p => p.sizes.some(s => s.isAvailable)).length : 0;
      if (count > 0) {
        tabs.push({
            ...
            {categoryTabs.map((tab) => {
              ...
              const tabProducts = menuDataAll ? (
                tab.id === "ALL" 
                  ...
                  : menuDataAll.find(m => String(m.categoryId) === tab.id)?.products || []
              ...
              return (
                <div key={tab.id} className="flex flex-col gap-1">
                  <button
                    onClick={() => setSelectedCategoryId(isExpanded ? "" : tab.id)}
                  ...
                  >
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      isExpanded ? "bg-amber-700/50 text-white" : "bg-stone-100 text-stone-500"
                    }`}>
                      {tab.count}
                    </span>
                  </button>
                  ...
                  {isExpanded && (
                    <div className="flex flex-col gap-1 pl-2 pr-1 animate-in slide-in-from-top-2 duration-200">
                      {tabProducts.map((p) => {
                        ...
                        const isActive = String(p.productId) === String(productDetailData?.productId);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-medium truncate ${isActive ? "text-amber-800" : "text-stone-700"}`}>
                                {p.name}
                              </p>
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  {availableDetailSizes.length > 1 && (
                    <span className="text-sm text-stone-400">
                      {formatPrice(
                        Math.min(...availableDetailSizes.map((s) => s.price)),
                      )}
                      {" ~ "}
                      {formatPrice(
                        Math.max(...availableDetailSizes.map((s) => s.price)),
                      )}
                    </span>
                  )}
                </div>
                      ...
                      {toppingsByFranchiseVisible.length > 0 ? (
                        toppingsByFranchiseVisible.map(
                          (topping: ProductListItem) => {
                            ...
                            const availableToppingSizes = topping.sizes.filter(
                              (size) => size.isAvailable,
                            );
                            const firstAvailableSize = availableToppingSizes[0];
                            if (!firstAvailableSize) return null;

                            const isChecked = Boolean(selectedToppingSizeId);
